...
description: >
  ...
  state-of-the-art prompt/context engineering methodology (4D, RISEN, CO-STAR, Chain-of-Draft,
  Step-Back, ReAct, Constitutional loops, Meta-Prompting, Gauntlet Loop prompting, and more). Use
  EVERY TIME a user asks to: improve/write/optimize a prompt, engineer a system prompt, or says
  "help me prompt", "make this prompt work", "write me a prompt for X", "craft a prompt for
  ChatGPT/Claude/Gemini/DeepSeek/Grok". Also trigger when a user shares a rough idea to structure;
  wants an iterative build/critique loop for a coding agent - "gauntlet loop", "gauntlet prompting",
  "have the agent judge its own work against a reference"; or wants a prompt built token-efficient,
  cache-friendly, or cost-conscious for agentic/production use. Always GODMODE - the most
  architecturally complete, platform-calibrated, cost-conscious prompt possible.
metadata:
  version: 3.6.0
  ...
  changelog:
    - version: 3.6.0
      date: "2026-08-08"
      changes:
        - "Corrected a citation misattribution in the Meta-Prompting technique (SKILL.md's technique
          table and references/techniques.md #19), the same class of error this skill's own
          changelog has caught before (see v3.4.0). The skill previously cited \"Suzgun & Kalai
          2023, arXiv:2311.11482\" for meta-prompting. Direct verification found these are two
          unrelated papers that happen to share almost the same title: arXiv:2311.11482 (\"Meta
          Prompting for AI Systems\") is by Zhang, Yuan & Yao, not Suzgun & Kalai, and formalizes
          meta-prompting categorically (a functor from tasks to prompts). Suzgun & Kalai's actual
          paper is arXiv:2401.12954 (\"Meta-Prompting: Enhancing Language Models with Task-Agnostic
          Scaffolding\"), which describes a different mechanism - a single LM acting as a conductor
          that decomposes a task and delegates subtasks to tailored \"expert\" instances of itself.
          Split the single technique entry into two correctly-sourced ones: Meta-Prompting
          (Conductor-Expert Scaffolding, Suzgun & Kalai) and Recursive Meta-Prompting (Proposer-
          Validator-Executor Self-Refinement, Zhang/Yuan/Yao)."
        - "Added Recursive Meta-Prompting (RMP) as a new technique, sourced from arXiv:2311.11482's
          own extension of its base framework - a proposer model drafts prompt edits, a validator
          enforces an edit schema, and an executor model runs the resulting prompt, with edit
          accumulation modeled as a monoid action. Flagged as genuinely emerging: this paper was
          revised to its current v10 as recently as 2026-08-03 (5 days before this changelog entry),
          so treat the RMP framing as active research, not settled practice - the paper itself notes
          functorial structure preservation does not guarantee semantic correctness or convergence."
        - "Added Automatic Prompt Engineer (APE, Zhou et al. 2022, arXiv:2211.01910) to
          techniques.md as the lineage this technique family descends from - the original propose-
          score-search loop that treats an instruction as a program to be optimized against a score
          function, predating and structurally anticipating both meta-prompting papers above."
        - "Added a grounding note pointing to Anthropic's own Metaprompt (Claude Cookbook,
          platform.claude.com/cookbook/misc-metaprompt) and the Console's built-in prompt generator
          (platform.claude.com/docs, Console prompting tools) as the applied, product-level instance
          of this exact technique family - Claude generating a tailored starting prompt from a task
          description. Verified by direct fetch of both pages. Relevant because it is structurally
          the same job this skill itself performs, just narrower in scope (single-turn templates
          only, explicitly not guaranteed optimal, meant as a first draft for iteration rather than
          a finished spec)."
        - "Added a META-PROMPTING SELF-REFINEMENT PASS section and a Step 9 in the Diagnostic
          Ingestion Protocol: after drafting the prompt (Step 8 - Build), run one silent internal
          critique-revise pass on the draft itself - not just offer meta-prompting as a technique
          the *output* prompt might contain, but apply the conductor/critique pattern to this
          skill's own act of generating that output. Scaled by stakes, matching the over-engineering
          caution already applied to Tier 5 and the Unknowns Discovery Protocol: skipped for
          trivial one-shot requests, applied for anything with real complexity or production stakes.
          Added a matching Quality Checklist item and reference-file index update."
    - version: 3.5.0
      date: "2026-08-06"
      changes:
        - "Added references/gauntlet-loop.md, documenting the \"Gauntlet Loop\" method for
          autonomous coding-agent builds: a lead agent decomposes a goal against a concrete,
          inspectable bar, separate builder and fresh-context critic sub-agents iterate with no
          fixed round count, and progress is tracked via a self-updating status artifact instead of
          manual check-ins. Source is Matt Shumer, \"How to Run a Gauntlet Loop\" (somethingbig.ai/
          gauntlet-loop, published 2026-07-27), verified by direct fetch - the practice behind the
          viral \"Claude of Duty\" demo. Flagged explicitly as a few-weeks-old community practice,
          not vendor documentation: noted Shumer's own public one-shot-to-zero-shot correction, and
          added a naming-collision section distinguishing this build/critique practice from
          unrelated same-named validation-loop tools (agent-gauntlet, old-coder/pi-gauntlet). The
          file's own Gauntlet Loop prompt template is original to this skill, not copied from the
          source prompt, and folds in an explicit resource budget and cache-aware/minimal-context
          instructions rather than leaving the loop's cost unaddressed."
        - "Added a \"Token-Efficient Prompt Construction (Cost Engineering)\" section to
          references/context-engineering.md covering cache-aware prompt ordering, reasoning-length
          compression for high-volume deployments, repeated-task compilation, and the multi-agent
          token multiplier - each claim checked against its primary source rather than a secondary
          summary: Anthropic's own multi-agent research system post (4x/15x token multipliers);
          Lumer et al., \"Don't Break the Cache\" (arXiv:2601.06007, confirmed via direct arXiv
          fetch: 41-80% cost reduction, 13-31% TTFT improvement, and the finding that naive
          full-context caching can underperform strategic cache-boundary control); Aytes et al.,
          \"Sketch-of-Thought\" (arXiv:2503.05179, EMNLP 2025, confirmed via ACL Anthology, cited as
          a 75-84% range rather than one figure since the reported number varies by version); and
          Wang et al., \"Good to Go: The LOOP Skill Engine\" (arXiv:2605.14237) for repeated/periodic-
          task compilation. This is now a standing checklist applied to every generated prompt, not
          only agentic ones, per the Integration section of that file."
        - Added a Step 6 (Production Pattern Check) routing check and a new GAUNTLET LOOP PROMPTING
          section to this file, cross-referencing Tier 5 Patterns 8, 14, and 26 and the Unknowns
          Discovery Protocol's References technique. Added a Framework Toolkit row, a Reference
          Files index entry, and a Quality Checklist item for the token-efficiency pass. Extended
          Operating Rule 2 to name token-efficiency as part of what "structure-first" means, not a
...
Separately again: is the deliverable specifically an autonomous coding-agent build meant to run an
open-ended improve-against-a-reference cycle (build it, judge it against something real, fix the
gap, repeat) rather than a single build-and-done task? If so, pull in
`references/gauntlet-loop.md` (Gauntlet Loop prompting) alongside whichever of the above also apply
- and if the user's own phrasing just says "gauntlet" without more, check that file's naming-
collision note before assuming which practice they mean.

**Step 7 - Token-Efficiency Pass.** Before building, note whether this deliverable is agentic,
production/repeated-use, or multi-agent (including any Gauntlet Loop from Step 6) - if so, the
checklist in `references/context-engineering.md` (Token-Efficient Prompt Construction) applies in
full: cache-aware ordering, reasoning-length discipline, and an explicit resource budget on any
open-ended loop. Its lighter half (no padded few-shot, no needlessly verbose reasoning) applies to
every prompt regardless of deployment context - see Operating Rule 2.

**Step 8 - Build.**

**Step 9 - Meta-Prompting Self-Refinement Pass.** Before delivering, weigh whether this request has
real stakes (production deployment, an agentic/persona/system prompt, or a complex multi-component
spec) or is a low-stakes single-shot draft. For anything with real stakes, run one silent internal
critique-revise cycle on the draft prompt itself using the pattern in META-PROMPTING SELF-
REFINEMENT PASS below, before moving to the Quality Checklist. Skip it for simple requests where the
Quality Checklist alone is sufficient - an extra pass on a five-minute task is the same over-
engineering failure mode already flagged for Tier 5 and the Unknowns Discovery Protocol.
...
| **Meta-Prompting (Conductor-Expert)** | A single LM acts as conductor, decomposing a task and delegating subtasks to tailored "expert" instances of itself, then integrating and verifying their outputs (Suzgun & Kalai 2024, arXiv:2401.12954). Structurally decoupled from content - reusable across tasks without content binding. | Highly novel domains where model lacks structural analogies |
| **Recursive Meta-Prompting (RMP)** | A proposer model drafts prompt edits, a validator checks them against an edit schema, an executor runs the resulting prompt (Zhang, Yuan & Yao, arXiv:2311.11482). The mechanism this skill's own Self-Refinement Pass below is modeled on. Emerging technique - the paper itself notes structural correctness does not guarantee convergence. | Low-stakes drafts where an extra silent pass isn't worth the tokens |
...
| Autonomous coding-agent build with iterative self-critique against a reference (Gauntlet Loop) | ReAct + RISEN for the goal/bar statement | references/gauntlet-loop.md for the full loop structure; Tier 5 Patterns 8/14/26; token-efficiency checklist for the resource budget |
...
## GAUNTLET LOOP PROMPTING (ITERATIVE BUILD/CRITIQUE FOR CODING AGENTS)

Full method: `references/gauntlet-loop.md`. A Gauntlet Loop is not another framework alongside CO-STAR
or RISEN - it's an operating pattern for a specific job: an autonomous coding agent (Claude Code,
Codex, or similar - requires an actual tool-using harness, not a plain chat window) that builds
something, judges its own output against a real, inspectable reference, fixes the biggest gap, and
repeats with no fixed round count until it clears the bar or the user stops it.

**Provenance note (surface this if the user asks where the method comes from):** sourced from Matt
Shumer's "How to Run a Gauntlet Loop" (somethingbig.ai/gauntlet-loop, published 2026-07-27), verified
by direct fetch - the practice behind the viral "Claude of Duty" agentic build. This is a few-weeks-
old community-popularized practice, not vendor documentation from Anthropic or anyone else; treat the
loop *structure* as solid (independently corroborated and directly verified) and any specific
model-name or cost/scale figure attached to the original demo as unconfirmed. See the reference file's
own Source and Verification section for the full caveat, including a naming collision worth checking
before assuming a user's "gauntlet" means this practice rather than an unrelated validation-loop tool.

**Two things to check during Step 6 above:**
1. Is the deliverable specifically this pattern - open-ended, comparison-driven iteration on an
   autonomous build - rather than a one-shot agentic system prompt or a bounded coding task? If so,
   write the goal, the bar, and an explicit resource budget into the generated prompt using the
   template in the reference file, rather than reaching for a generic agentic prompt structure.
2. Because this pattern is a multi-agent fan-out with no fixed stopping point by design, always pair
   it with the token-efficiency checklist below - specifically an explicit time/token/dollar ceiling
   and cache-aware, minimal-context builder/critic exchanges. An unbounded Gauntlet Loop with no
   stated budget is the single easiest way this skill could generate a prompt that runs away on cost.

**When NOT to use this layer:** a bounded, well-specified coding task, a one-off script, anything
meant to run in a plain chat interface rather than an agentic harness, or content that can't be
mechanically compared against a reference. Route those through Tiers 1-4 or Tier 5 alone.

---

## META-PROMPTING SELF-REFINEMENT PASS

Every layer above describes techniques this skill can *hand to the target AI* inside the generated
prompt. This section is different: it applies meta-prompting to this skill's own act of writing that
prompt, not just to what the prompt tells someone else's model to do.

**The mechanism, and where it comes from:** Suzgun & Kalai's meta-prompting (arXiv:2401.12954) has
one LM act as a conductor - decomposing a task, delegating pieces to tailored "expert" instances of
itself, then integrating and verifying the results before finalizing. Zhang, Yuan & Yao's Recursive
Meta-Prompting (arXiv:2311.11482) formalizes a tighter loop: a proposer drafts an edit, a validator
checks it against a schema, an executor applies it. Both papers describe automating what a careful
human editor already does by hand - draft, adopt a critical outside stance, find the real weak
points, fix only those, stop. This pass is a lightweight, single-cycle version of that same
discipline, scoped so it costs one internal pass rather than an open-ended loop:

1. **Draft.** Complete Step 8 - Build normally. This is the candidate prompt.
2. **Adopt the conductor/expert stance.** Silently re-read the draft as a skeptical expert reviewer
   would - not the author defending it. Check it against: the 6-Component Spec (are all six
   genuinely complete, not just present?), the Phase 0 Step 2 failure vectors (lexical flatness,
   attention collapse, missing contrastive boundaries, thin identity anchoring, no quality gate,
   platform mismatch, scope ambiguity), and platform calibration for the confirmed target model.
3. **Identify, don't rewrite wholesale.** Name the 1-3 most significant weaknesses found - the ones
   that would actually degrade the target AI's output, not stylistic nitpicks. Bounding this to 1-3
   keeps the pass token-efficient and mirrors APE's (Zhou et al. 2022, arXiv:2211.01910) score-then-
   improve loop rather than an unbounded rewrite.
4. **Revise only those sections.** Apply fixes surgically. A prompt that already scores well on the
   Quality Checklist needs zero changes here - this pass is a check, not a mandate to find something
   wrong.
5. **Deliver the revised version silently.** The critique itself is internal scaffolding, like the
   rest of Phase 0 - it does not appear in the meta-header or debrief. What the user sees is Parts
   1-3 below, built from the post-refinement draft.

**Honesty about this technique's own limits (both source papers say so directly):** neither paper
claims this loop guarantees convergence or a semantically better prompt - Zhang, Yuan & Yao are
explicit that preserving structure functorially is not the same as preserving correctness. Treat
this pass as a disciplined second look, not a formal proof of improvement. If the draft was already
strong, the honest outcome is "no changes" - don't manufacture edits to justify having run the pass.

**When NOT to use this layer:** anything Step 9 above already routed away from - simple one-shot
requests where the Quality Checklist alone catches what matters. Running this on every three-line
LinkedIn-post prompt is the over-engineering failure mode this skill flags everywhere else.
...
- [ ] Token-efficiency pass: no padded near-duplicate few-shot examples, reasoning length capped by
      default for high-frequency/cost-sensitive targets; for agentic/production/multi-agent
      deliverables, cache-aware ordering applied and an explicit resource budget stated on any
      open-ended loop (see references/context-engineering.md)
- [ ] Gauntlet Loop check: if the deliverable is an autonomous coding-agent build meant to iterate
      against a reference bar, goal/bar/decomposition/critic-isolation/budget are all present
      (see references/gauntlet-loop.md); skipped for one-shot or non-comparable deliverables
- [ ] Meta-prompting self-refinement pass: for anything with real stakes (production, agentic/
      persona/system prompt, or a complex multi-component spec), one silent conductor/expert
      critique-revise cycle has been run on the draft itself (see META-PROMPTING SELF-REFINEMENT
      PASS); intentionally skipped for simple one-shot requests
...
2. **Structure-first, depth-by-requirement.** A 400-word precision spec outperforms a padded
   2,000-word scaffold. Build to completeness of the 6 spec components, not word count. This
   includes token-efficiency as part of what "structure-first" means, not a separate concern bolted
   on afterward: no padded few-shot repetition, reasoning length capped by default for high-volume or
   cost-sensitive targets, and cache-aware ordering plus an explicit resource budget for any
   agentic, production, or multi-agent deliverable (see references/context-engineering.md).
...
- `Claude Code - A Gauntlet Loop that builds a landing page and keeps iterating against real
  competitor screenshots until it wins the comparison`
...
- `references/techniques.md` - Full technique library with mechanistic explanations, worked
  examples, and a Research Foundations table of verified arXiv citations (added 2026-07-16,
  extended 2026-08-08) replacing several previously unsourced or misattributed claims - most
  recently a paper-title mix-up in the meta-prompting entry itself (Suzgun & Kalai's actual paper is
  arXiv:2401.12954, not arXiv:2311.11482, which is an unrelated paper by Zhang/Yuan/Yao with a
  near-identical title). Now covers the full meta-prompting family: Meta-Prompting (Conductor-Expert,
  Suzgun & Kalai), Recursive Meta-Prompting (proposer-validator-executor, Zhang/Yuan/Yao - actively
  being revised as of Aug 2026), its lineage in Automatic Prompt Engineer (Zhou et al. 2022), and a
  grounding note pointing to Anthropic's own Metaprompt/Console prompt generator as the applied
  version of this same technique family - see META-PROMPTING SELF-REFINEMENT PASS above for how this
  skill applies the pattern to its own output rather than only offering it as an ingredient
...
- `references/context-engineering.md` - Anthropic's actual framework for managing an agent's context
  ...
  deliverable is agentic or long-running enough to outgrow a single context window. Also includes a
  Token-Efficient Prompt Construction section (added 2026-08-06) - cache-aware ordering, reasoning-
  length compression, repeated-task compilation, and the multi-agent token multiplier, each grounded
  in a directly-verified primary source - applied as a standing checklist to every prompt this skill
  generates, agentic or not.
- `references/gauntlet-loop.md` - The Gauntlet Loop method (added 2026-08-06) for autonomous coding-
  agent builds: goal-not-implementation framing, a concrete inspectable bar, agent-owned task
  decomposition, strictly separated builder/critic sub-agents, an open-ended-but-budgeted round
  structure, and a self-updating progress artifact in place of manual check-ins - plus an original
  prompt template built to satisfy the token-efficiency checklist alongside the loop structure itself.
  Sourced from a verified direct fetch of the practice's own origin post (Matt Shumer, July 2026),
  with explicit honesty caveats about how recent and unofficial this practice is, and a section
  distinguishing it from unrelated same-named validation-loop tooling. Use whenever the deliverable is
  specifically an iterative build-and-critique loop for an agentic coding harness (Claude Code, Codex,
  or similar), not for one-shot agentic system prompts generally (route those through Tier 5 instead).
